                        ...
                        ram_addr_t size, ram_addr_t phys_offset)
{
    ...
    DPRINTF("%s(%04x:%02x:%02x.%x) 0x%lx[0x%lx] -> 0x%lx\n",
            __FUNCTION__, vdev->host.seg, vdev->host.bus, vdev->host.dev,
                          ...
                          ram_addr_t size, ram_addr_t phys_offset)
{
    ...
    if (ioctl(vdev->vfiofd, VFIO_DMA_UNMAP, &dma_map)) {
        DPRINTF("VFIO_DMA_UNMAP: %d\n", errno);
        return -errno;
    }
...
static int vfio_parse_netlink(struct nl_msg *msg, void *arg)
{
    ...
    seg = nla_get_u32(attrs[VFIO_ATTR_PCI_DOMAIN]);
...
static int vfio_register_netlink(VFIODevice *vdev)
{
    ...
    if (QLIST_EMPTY(&nl_list)) {
        ...
        nl_socket_modify_cb(vfio_nl_handle, NL_CB_VALID, NL_CB_CUSTOM,
    ...
    nla_put_u32(msg, VFIO_ATTR_PCI_DOMAIN, vdev->host.seg);
    nla_put_u16(msg, VFIO_ATTR_PCI_BUS, vdev->host.bus);
...
static int enable_uiommu(VFIODevice *vdev)
{
    ...
    if (ioctl(vdev->vfiofd, VFIO_DOMAIN_SET, &fd)) {
        fprintf(stderr, "%s: Failed VFIO_DOMAIN_SET: %s\n",
