...
/*
 * Both newer Radeon HD cards (HD5450 & HD7850) as well as older Radeon X
 * cards (X550) report the upper byte of the I/O port BAR through VGA register
 * 0x3c3.  The difference between new and old is which BAR is the I/O port
 * BAR.  On newer cards it's BAR4 on older cards it's BAR1.
 */
...
static void vfio_vga_probe_ati_3c3_quirk(VFIODevice *vdev)
{
    ...
    if (vdev->bars[4].ioport && vdev->bars[4].size >= 256) {
        nr = 4;
    } else if (vdev->bars[1].ioport && vdev->bars[1].size >= 256) {
        nr = 1;
    } else {
        return;
    }
    ...
    if (pread(vdev->fd, &physbar, 4,
              vdev->config_offset + PCI_BASE_ADDRESS_0 + (4 * nr)) != 4) {
    ...
    quirk->vdev = vdev;
    quirk->data.base_offset = 3;
    quirk->data.address_match = (physbar >> 8) & 0xff;
    quirk->data.address_val = PCI_BASE_ADDRESS_0 + (4 * nr) + 1;
    ...
    DPRINTF("Enabled ATI/AMD quirk 0x3c3 BAR%d for device %04x:%02x:%02x.%x\n",
            nr, vdev->host.domain, vdev->host.bus, vdev->host.slot,
            vdev->host.function);
}
...
static void vfio_probe_ati_bar4_window_quirk(VFIODevice *vdev, int nr)
{
    ...
    memory_region_init_io(&quirk->mem, &vfio_generic_window_quirk, quirk,
...
/*
 * On older ATI/AMD cards (X550) I/O port BAR1 provides a window to a
 * mirror of PCI config space located in BAR2 at offset 0xf00.  Quirk
 * both the window and direct access.
 */
static void vfio_probe_ati_bar1_window_quirk(VFIODevice *vdev, int nr)
{
    PCIDevice *pdev = &vdev->pdev;
    VFIOQuirk *quirk;

    if (!vdev->has_vga || nr != 1 ||
        pci_get_word(pdev->config + PCI_VENDOR_ID) != PCI_VENDOR_ID_ATI) {
        return;
    }
    ...
    quirk->data.data_offset = 4;
    quirk->data.data_size = 4;
    quirk->data.address_match = 0xf00;
    quirk->data.address_mask = PCI_CONFIG_SPACE_SIZE - 1;
    quirk->data.bar = nr;

    memory_region_init_io(&quirk->mem, &vfio_generic_window_quirk, quirk,
                          "vfio-ati-bar1-window-quirk", 8);
    memory_region_add_subregion_overlap(&vdev->bars[nr].mem,
                          quirk->data.base_offset, &quirk->mem, 1);

    QLIST_INSERT_HEAD(&vdev->bars[nr].quirks, quirk, next);

    DPRINTF("Enabled ATI/AMD BAR1 window quirk for device %04x:%02x:%02x.%x\n",
            vdev->host.domain, vdev->host.bus, vdev->host.slot,
            vdev->host.function);
}

static void vfio_probe_ati_bar2_f00_quirk(VFIODevice *vdev, int nr)
{
    PCIDevice *pdev = &vdev->pdev;
    VFIOQuirk *quirk;

    /* Only enable on older devices where BAR2 is 32bit */
    if (!vdev->has_vga || nr != 2 || vdev->bars[2].mem64 ||
        pci_get_word(pdev->config + PCI_VENDOR_ID) != PCI_VENDOR_ID_ATI) {
        return;
    }
    ...
    DPRINTF("Enabled ATI/AMD BAR2 0xf00 quirk for device %04x:%02x:%02x.%x\n",
            vdev->host.domain, vdev->host.bus, vdev->host.slot,
            vdev->host.function);
}
...
static void vfio_bar_quirk_setup(VFIODevice *vdev, int nr)
{
    ...
    vfio_probe_ati_bar1_window_quirk(vdev, nr);
    vfio_probe_ati_bar2_f00_quirk(vdev, nr);
